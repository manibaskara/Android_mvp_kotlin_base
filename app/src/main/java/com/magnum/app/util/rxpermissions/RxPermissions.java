package com.magnum.app.util.rxpermissions;
...
public class RxPermissions {
    ...
    protected static final String TAG = "RxPermissions";
    ...
    public Observable.Transformer<Object, Boolean> ensure(final String... permissions) {
        return o -> request(o, permissions)
                ...
                .flatMap((Func1<List<Permission>, Observable<Boolean>>) permissions1 -> {
                    // Occurs during orientation change, when the subject receives onComplete.
                    // In that case we don't want to propagate that empty list to the
                    // subscriber, only the onComplete.
                    if (permissions1.isEmpty()) return Observable.empty();
    ...
    Observable<Permission> request(final Observable<?> trigger, final String... permissions) {
        ...
        return oneOf(trigger, pending(permissions)).flatMap((Func1<Object, Observable<Permission>>) o -> requestImplementation(permissions));
    }

    private Observable<?> pending(final String... permissions) {
        for (String p : permissions)
            if (!mRxPermissionsFragment.containsByPermission(p)) return Observable.empty();
        return Observable.just(null);
    }

    private Observable<?> oneOf(Observable<?> trigger, Observable<?> pending) {
        if (trigger == null) return Observable.just(null);
    ...
    public Observable<Boolean> shouldShowRequestPermissionRationale(final Activity activity, final String... permissions) {
        if (!isMarshmallow()) return Observable.just(false);
    ...
    private boolean shouldShowRequestPermissionRationaleImplementation(final Activity activity, final String... permissions) {
        for (String p : permissions)
            if (!isGranted(p) && !activity.shouldShowRequestPermissionRationale(p)) return false;
        return true;
    }
